...
<div @if ($scan->isRunning()) wire:poll.750ms @endif class="py-8">
    ...
    @else
        ...
        {{-- Monitoring: alert me if this grade drops --}}
        <div class="mt-8 rounded-2xl border border-zinc-200 bg-zinc-50/50 p-5 dark:border-zinc-800 dark:bg-zinc-900/30">
            @if ($monitor && $monitor->isVerified())
                {{-- State 3: verified and watching --}}
                <div class="flex items-start gap-3">
                    <svg viewBox="0 0 24 24" fill="none" class="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M20 6L9 17l-5-5"/>
                    </svg>
                    <div>
                        <p class="text-sm text-zinc-700 dark:text-zinc-200">
                            Verified — you’re watching <span class="font-semibold">{{ $scan->host }}</span>.
                            We’ll email <span class="font-semibold">{{ $monitor->email }}</span> if its grade drops below {{ $monitor->last_grade_letter }}.
                        </p>
                        <a href="{{ route('monitor.manage', $monitor) }}" wire:navigate class="mt-1 inline-block text-xs font-medium text-zinc-500 underline underline-offset-2 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">
                            Manage or unsubscribe
                        </a>
                    </div>
                </div>
            @elseif ($monitor)
                {{-- State 2: registered, awaiting domain verification --}}
                <p class="text-sm font-semibold">One step left: verify you own {{ $scan->host }}</p>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                    So we only monitor sites their owners control, add <span class="font-medium">any one</span> of these, then verify.
                </p>

                <div class="mt-4 space-y-3 text-sm">
                    <div class="rounded-xl border border-zinc-200 bg-white p-3.5 dark:border-zinc-800 dark:bg-zinc-900">
                        <p class="text-xs font-semibold uppercase tracking-wide text-zinc-400">Option A — DNS TXT record</p>
                        <p class="mt-1.5 break-all font-mono text-xs text-zinc-700 dark:text-zinc-200">{{ $verificationValue }}</p>
                    </div>
                    <div class="rounded-xl border border-zinc-200 bg-white p-3.5 dark:border-zinc-800 dark:bg-zinc-900">
                        <p class="text-xs font-semibold uppercase tracking-wide text-zinc-400">Option B — homepage meta tag</p>
                        <p class="mt-1.5 break-all font-mono text-xs text-zinc-700 dark:text-zinc-200">&lt;meta name="{{ $verificationName }}" content="{{ $monitor->verification_token }}"&gt;</p>
                    </div>
                    <div class="rounded-xl border border-zinc-200 bg-white p-3.5 dark:border-zinc-800 dark:bg-zinc-900">
                        <p class="text-xs font-semibold uppercase tracking-wide text-zinc-400">Option C — well-known file</p>
                        <p class="mt-1.5 break-all font-mono text-xs text-zinc-700 dark:text-zinc-200">/.well-known/{{ $verificationName }}.txt → {{ $monitor->verification_token }}</p>
                    </div>
                </div>
                ...
                <div class="mt-4 flex items-center gap-3">
                    <button
                        ...
                        wire:target="verify"
                        class="inline-flex items-center justify-center rounded-xl bg-[#F53003] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#d62902] focus:outline-none focus:ring-2 focus:ring-[#F53003]/30 disabled:opacity-60"
                    >
                        <span wire:loading.remove wire:target="verify">I’ve added it — verify</span>
                        <span wire:loading wire:target="verify">Checking…</span>
                    </button>
                    <span class="text-xs text-zinc-400">Watching for {{ $monitor->email }}</span>
                </div>
                @error('verify')
                    <p class="mt-2.5 text-sm text-[#F53003]">{{ $message }}</p>
                @enderror
            @else
                {{-- State 1: not watching yet --}}
                <p class="text-sm font-semibold">Get alerted if this grade drops</p>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                <form wire:submit="watch" class="mt-4 flex flex-col gap-2.5 sm:flex-row">
                    <input
                        ...
                        autocomplete="email"
                        class="flex-1 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm shadow-sm outline-none transition placeholder:text-zinc-400 focus:border-[#F53003] focus:ring-2 focus:ring-[#F53003]/20 dark:border-zinc-800 dark:bg-zinc-900 dark:placeholder:text-zinc-600"
                    >
                    <button
                        type="submit"
                        class="inline-flex items-center justify-center rounded-xl bg-[#F53003] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#d62902] focus:outline-none focus:ring-2 focus:ring-[#F53003]/30 disabled:opacity-60"
                        ...
                        wire:target="watch"
                    >
                        <span wire:loading.remove wire:target="watch">Watch this site</span>
                        <span wire:loading wire:target="watch">Saving…</span>
                    </button>
                </form>
                @error('email')
