...
The text the user typed after `/speckit.specify` **is** the feature description. Assume it is available even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
3. **Limit clarifications**: Maximum 3 [NEEDS CLARIFICATION] markers (per the execution flow), prioritized scope > security/privacy > UX > technical details
