...
description: Create a repository interface in Domain with Eloquent and InMemory implementations
---
...
{
    public function save(<EntityName> $entity): void {
        <EntityName>Model::updateOrCreate(['id' => $entity->id()->value()], $this->toArray($entity));
    }

    public function findById(<EntityName>Id $id): ?<EntityName> {
        $model = <EntityName>Model::find($id->value());
        return $model ? $this->toDomain($model) : null;
    }

    public function delete(<EntityName>Id $id): void {
        <EntityName>Model::destroy($id->value());
    }
...
Integration tests use `Tests\TestCase` with the `RefreshDatabase` trait since they hit the real database.
