...
4. If brownfield or broad enough to need codebase inspection, review local context first: Grep/Glob/Read over `modules/{Module}/{Domain,Application,Infrastructure}` and `resources/frontend/src/`; routes via `php artisan route:list`; schema in `database/migrations/`.
